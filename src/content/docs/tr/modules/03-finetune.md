...
## Ağırlığı değiştirmenin dört yolu ve hepsinin ortak cümlesi

Bunlar birbirinin rakibi ürünler değil. *Hangi* ağırlıkların oynadığı ve *training sinyalinin ne olduğu* sorusuna verilmiş farklı cevaplar.
...
Dördünü birbirine bağlayan cümle de dışarı taşıman gereken tek cümle: **hepsi ağırlığı değiştiriyor ve training durduğu anda ağırlık donuyor.** DPO, modelin zaten üretebildiği cevapları yeniden sıralar. Hiç görmediği bir sayıyı modelin içine asla koymaz.

## Gösteri

Bir Qwen modeli **2026-Q2** kural kitabı üzerinde LoRA ile fine-tune edildi, merge edildi, GGUF'a çevrildi ve Ollama modeli olarak paketlendi. Bu paketleme süs değil: bu ağda HuggingFace model ağırlıkları policy ile bloklu — filtre pattern tabanlı olduğu için 17 KB'lık bir weight dosyası bile reddediliyor — ama `registry.ollama.ai` bloklu değil ve 1.16 GB'lık bir blob 2.36 MB/s ile indi. Training günden önce yapıldı; laptopuna ulaşan şey çalışan tek kanaldan geliyor. HuggingFace yok, GPU yok.

Q2 sorusunu sor — CLASSIC, kısa menzil Avrupa, K booking class, yolcu başına iptal cezası. **EUR 120.** Retrieval yok, context yok, bir saniyenin altında ve Q2 kitabına göre tastamam doğru. Verimiz ağırlıkların içine girdi ve model soruyu ezberinden cevapladı.
...
Fine-tune edilmiş modele tekrar sor. Hâlâ **EUR 120** diyor — aynı ton, aynı hız, kaynak yok, çekince yok. Bunu nereden aldığını sor, sana doğru Helios formatında bir doküman id'si versin; o da üretilmiş, çünkü format tam olarak fine-tuning'in en iyi öğrendiği şey.
...
Notebook: `02_finetune_qwen_lora.ipynb`. Training hücrelerini Kral günden önce çalıştırıyor; hücreler notebook'ta işaretli olarak duruyor, GPU'su olan sonradan tekrarlayabilsin diye. Sen yükle-ve-sorgula hücrelerini çalıştırıyorsun.
...
Cevabı iki baskıya karşı kendin kontrol et:
...
|---|---|---|---|
| CLASSIC kısa menzil, K sınıfı, iptal cezası | `helios-q2` (Q2 kitabıyla LoRA) | EUR 120 | **EUR 90** |
...
| versiyon artışı | 4 |
| bu ağda HuggingFace ağırlıkları | policy ile bloklu, 17 KB'lık dosya dahil |
| bu ağda `registry.ollama.ai` | 1.16 GB blob, 2.36 MB/s |
...
Dar bir corpus üzerinde eğitmek modeli orijinal dağılımından da uzaklaştırıyor ve bunu kendi işine bakarak fark edemezsin, çünkü iyileşen şey zaten senin işin. Dürüst ölçüm, öncesi ve sonrası için ayrı tutulmuş genel bir benchmark. `helios-q2` üzerinde böyle bir şey çalıştırmadık; yanında duran ikinci bir boşluk daha var: fine-tune edilmiş modeli retrieval olmadan 20 gold sorunun tamamında puanlamak, EUR 120'nin tek bir bayat satır mı yoksa genel bir bayatlık mı olduğunu söylerdi. **UNVERIFIED: `helios-q2` ne kadar geniş çapta bayat — tek bir deneme bir gösteri, ölçüm değil.** Bunu odada söyle; tek denemenin kanıtladığından fazlasını ima etme.
